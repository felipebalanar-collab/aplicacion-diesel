...
import { Search, Settings, Activity, PenTool, ArrowRight, Info } from "lucide-react";
...
function HomeChild() {
  ...
  const [authUser, setAuthUser] = useState<{ nombre?: string; email?: string; rol?: string } | null>(null);
  ...
  // Auto-load results when all 3 parameters are present (on page load from URL)
  useEffect(() => {
    const autoLoadResults = async () => {
      if (fuel && category && family) {
        console.log(`[CLIENT] Auto-loading with fuel="${fuel}", cat="${category}", fam="${family}"`);
        setIsSearching(true);
        try {
          const data = await searchInjectors("", fuel as any, category as any, family as any);
          console.log(`[CLIENT] Auto-loaded ${data.length} results`);
          setResults(data);
        } catch (error) {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      console.log(`[CLIENT] Search effect triggered: query="${searchQuery}", fuel="${fuel}", cat="${category}", fam="${family}"`);
      
      if (searchQuery.length > 1 || (fuel && category && family)) {
        console.log(`[CLIENT] Calling searchInjectors...`);
        setIsSearching(true);
        ...
        console.log(`[CLIENT] Parameters: query="${query}", fuel="${currentFuelType}", cat="${currentCategory}", fam="${currentFamily}"`);
        
        const data = await searchInjectors(query, currentFuelType as any, currentCategory as any, currentFamily as any);
        
        console.log(`[CLIENT] Received ${data.length} results`);
        setResults(data);
        setIsSearching(false);
      } else {
        console.log(`[CLIENT] Conditions not met, clearing results`);
        setResults([]);
      }
    }, 300);
    ...
    <div className="space-y-12 pb-20">
      {authUser && (
        <section className="max-w-4xl mx-auto px-4">
          <div className="bg-slate-800/60 border border-white/10 rounded-xl p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
              <div className="text-xs text-slate-400 uppercase tracking-widest">Sesion iniciada</div>
              <div className="text-sm text-white font-semibold">
                {authUser.nombre || authUser.email}
              </div>
            </div>
            <div className="flex items-center gap-2">
              {isAdmin ? (
                <>
                  <span className="text-xs bg-emerald-500/20 text-emerald-400 px-2 py-1 rounded font-bold uppercase">
                    Modo Administracion
                  </span>
                  <Link
                    href="/admin/usuarios"
                  ...
                  <button
                    ...
                    className="text-xs bg-rose-600/80 hover:bg-rose-600 px-3 py-1 rounded transition"
                  >
                    Cerrar sesion
                  </button>
                </>
      ...
      <section className="max-w-5xl mx-auto px-4">
        <div className="glass-card p-4 md:p-5 border-white/10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="text-xs text-slate-400 uppercase tracking-widest">Accesos rapidos</div>
          <div className="flex flex-wrap gap-3">
            {isAdmin && (
              <Link
                href="/admin/usuarios"
                className="bg-slate-800 hover:bg-slate-700 border border-white/10 px-4 py-2 rounded-lg text-xs font-bold industrial-text"
              >
                Admin usuarios
              </Link>
            )}
            <Link href="/docs" className="bg-slate-800 hover:bg-slate-700 border border-white/10 px-4 py-2 rounded-lg text-xs font-bold industrial-text">
              MANUALES TECNICOS
            </Link>
            <Link href="/hardware-tips" className="bg-slate-800 hover:bg-slate-700 border border-white/10 px-4 py-2 rounded-lg text-xs font-bold industrial-text">
              HARDWARE TIPS
            </Link>
            <Link href="/sugerencias" className="bg-slate-800 hover:bg-slate-700 border border-white/10 px-4 py-2 rounded-lg text-xs font-bold industrial-text">
              SUGERENCIAS
            </Link>
          </div>
        </div>
      ...
      <section className="max-w-5xl mx-auto px-4">
        <div className="glass-card p-4 md:p-5 border-white/10 flex items-center justify-between">
          <div className="text-xs text-slate-400 uppercase tracking-widest">Soporte rapido</div>
          <div className="flex items-center gap-3">
            <Link href="/docs" className="text-xs text-sky-400 hover:text-sky-300 font-bold uppercase tracking-widest">
              Manuales
            </Link>
            <span className="text-slate-600">•</span>
            <Link href="/hardware-tips" className="text-xs text-emerald-400 hover:text-emerald-300 font-bold uppercase tracking-widest">
              Hardware
            </Link>
            <span className="text-slate-600">•</span>
            <Link href="/sugerencias" className="text-xs text-amber-400 hover:text-amber-300 font-bold uppercase tracking-widest">
              Sugerencias
            </Link>
          </div>
        </div>
      ...
      {isAdmin && (
        <Link 
          href="/admin/docs"
          className="fixed bottom-6 right-6 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-4 py-3 rounded-full shadow-2xl flex items-center gap-2 transition-all hover:scale-105 z-50 group"
          title="Panel de Administración"
        >
          <Settings className="w-5 h-5" />
          <span className="font-bold text-sm hidden group-hover:inline">Admin</span>
        </Link>
      )}
    </div>
  );
